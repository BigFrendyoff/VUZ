...
def decrypt(v, k):
    v0 = numpy.uint32(v[0])
    v1 = numpy.uint32(v[1])
    sum = numpy.uint32(0xC6EF3720)
    dt = numpy.uint32(0x9E3779B9)
    for i in range(32):
        v1 -= numpy.uint32(((v0 << 4) + k[2]) ^ (v0 + sum) ^ ((v0 >> 5) + k[3]))
        v0 -= numpy.uint32(((v1 << 4) + k[0]) ^ (v1 + sum) ^ ((v1 >> 5) + k[1]))
        sum -= dt
    print(hex(v0), hex(v1))
